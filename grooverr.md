...
21. **The actual root cause of "Requested format is not available", traced through yt-dlp's own source and consistent with every observation from every round.** The decisive report from the real Unraid host: Grooverr fails with that exact error on *ANY* media, while `docker exec -it Grooverr yt-dlp --list-formats <url>` — **inside the same container** — succeeds, landing on the `android_vr` client. That single fact eliminated image staleness, yt-dlp version, deno, network/IP, and permissions in one stroke, leaving only round 6's finding: the sole structural difference between the two invocations is `cookiefile`.
    ...
    - **Resolution status — this item supersedes the earlier investigation rounds for this bug.** Items 13 (format-selector rigidity), 17 (missing JS runtime / deno, and the PO-token research), 18 (stale deployed image), and 20 (cookiejar write-back discarding successful downloads) were each real findings fixed on their own merits — the deno install closes a genuine documented yt-dlp gap, the write-back fix stops a genuine job-discarding bug, the stale-image discovery produced the version footer (item 19), and the stuck failed queue items this bug created are what prompted the removal/cleanup feature (Section 7.6, item 16). But none of them was the cause of the recurring "Requested format is not available" failures; this item is. The tell that should have shortcut three rounds of investigation: yt-dlp's own "Skipping client" diagnostic warning fires at exactly the moment the authenticated client swap happens, and Grooverr's `no_warnings=True` silenced it — when a wrapped tool misbehaves inexplicably, unsilence its own diagnostics *first*, before diffing environments.
