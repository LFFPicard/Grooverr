...
15. **Bulk "Add entire discography" blocks and re-pays on retry** — measured live at ~447 sequential MusicBrainz requests / ~7.5 minutes minimum for a 219-release artist, held inside one synchronous HTTP request, head-of-line-blocking interactive search behind the shared rate limiter the whole time. A retry after partial failure re-resolved everything from scratch rather than skipping already-succeeded albums. **Resolved and verified live (2026-07-15):** bulk add now enqueues one `album_add` QueueItem per not-already-handled release (new `JobType.album_add`, carrying the full `ResolvedAlbum` as a JSON payload since no `Track`/`Album` row exists yet) and returns immediately; a worker resolves each one via the exact same `_add_album()` a manual single-album add already used (Section 7.5's Queue UI shows real progress). Verified against the real Linkin Park discography (219 releases): the endpoint returned in **30s** (not ~7.5 minutes), and interactive search stayed responsive throughout (~3s per search, vs. being blocked for the entire bulk duration before).

    Two further bugs were caught by insisting on live re-verification rather than trusting the architecture change alone — both are the kind of thing a mocked/small-scale test wouldn't surface: (a) the first dedup pass only checked *finished* releases (an `Album` row already exists) — retrying while the first run was still mid-flight silently double-enqueued every not-yet-finished release (101 duplicate jobs out of 219, confirmed live); fixed by also checking `queued`/`active` `album_add` jobs, keyed on `release_group_id` from each job's own payload. (b) Fixing (a) with a title-string check alone still perpetually re-enqueued 14 releases *forever* — anniversary editions, remix singles, box sets — because the canonical *release* `_release_rank` picks can carry a different title than its release-group's browse-time title (e.g. release-group "Hybrid Theory (20th anniversary edition)" resolves to the earliest official release, titled plain "Hybrid Theory"); the job completes successfully but a title match against the browse-time title never recognises it as handled. Fixed by keying the "already handled" check purely on each job's own `release_group_id` (queued/active/**done**, excluding only `error`), never on the resulting title — confirmed live with three consecutive retries against the real discography all returning `jobs_enqueued: 0` in ~8s once every release was genuinely handled.

The other 4 findings (MusicBrainz browse KeyError recurrence, two silently-ignored Settings fields, missing filename length cap, non-atomic M3U8 writes) were fixed and verified directly during the audit — see commits b54a80a and 69eec99, and the full findings list for detail. One item remains genuinely unverified rather than resolved either way: the non-root container check (Section 9/Batch 9) couldn't be re-confirmed live this session because the Docker daemon wasn't running (checked twice, including after the bulk-add work) — last live proof is Batch 9's. Worth a quick recheck next time Docker Desktop is up, before treating it as re-confirmed post-audit.
