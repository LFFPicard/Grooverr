...
2. Display as a card grid (reuse the Library album-card component and completeness-badge pattern — Section 8) grouped or filterable by release type (album / single / EP / compilation)
...
### Resolved during implementation of items 9-12 (2026-07-14)

13. **`inc=releases` 400s on the release-group browse resource** — implemented `browse_release_groups_by_artist` assuming (and mock-testing) that `inc=releases` would embed each release-group's member releases in one request, the same way it works on a release-group *lookup*. Live testing against the real MusicBrainz API (Linkin Park's discography) caught that browse rejects it: `"releases is not a valid inc parameter for the release-group resource"`. A MockTransport test alone could never have caught this — it validates the client's own shape, not MusicBrainz's actual constraints. **Resolved:** browse-by-artist no longer resolves a release id at browse/list time; `ResolvedAlbum` gained a `release_group_id` field, and the canonical member release (still MBID-based, still `_release_rank`-picked, never a text search) is resolved lazily via a new `browse_releases_by_release_group()` call only when an item is actually added — one extra request at add-time rather than per item on every page load, the same lazy-resolution latency tradeoff already accepted for search-result album summaries. "Already in library" on the discography list correspondingly changed to a title match within the artist (no release id exists yet to compare at browse time) — the same fallback heuristic `_find_or_create_album` already uses.
14. **Rate limiter audit outcome** — the audit required by item 11 found the limiter was *already* a single shared instance in production (`app.runtime.resolver.mb`, threaded through to the worker pipeline via `main.py`'s explicit `Pipeline(resolver=runtime.resolver)`); no refactor was needed. Added a regression test spying on `Pipeline.__init__` during a real app lifespan startup to catch a future accidental dropped kwarg, plus the concurrent-timing test Batch 10's DoD requires (search + a multi-page discography browse sharing one client, real request timestamps logged and asserted ≥~1s apart).
