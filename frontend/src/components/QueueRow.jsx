...
function statusPill(job) {
  ...
  const short = message.length > 28 ? `${message.slice(0, 28)}…` : message
  return (
...
export function QueueRow({ job }) {
  const progress = job.status === 'done' ? 100 : job.progress_percent || 0
  const barClass = job.status === 'done' ? 'bg-sage' : 'bg-mustard'

  return (
    <div className="grid grid-cols-[44px_1fr_130px_92px] items-center gap-3.5 px-5 py-3 border-b border-border last:border-b-0">
